function TrialInfo = ASF_showTrialMaskedPrimingUpperLower(atrial, windowPtr, Stimuli, Cfg)
%function TrialInfo = ASF_showTrialSample(atrial, windowPtr, Stimuli, Cfg)
%
%%DERIVED FROM ASF_showTrialSample
%% USES 5 USERDEFINED STM COLUMNS IN ORDER TO PRESENT PRIMES AND MASKS AT
%% DEFINED VERTICAL OFFSETS
%% MAKE SURE TO CREATE APPROPRIATE TRD (see makeExampleUpperLowerTrd.m)
%% MAKE SURE TO USE PROPER Cfg.userDefinedSTMcolumns = 6 (see runExampleUpperLower.m)
%
%% userDefined 1:   primePage
%% userDefined 2:   maskPage
%% userDefined 3-6: targetRect for primes and masks [ULrow, ULcol, LRrow, LRcol]
...
for i = 1:atrial.startRTonPage-1
    if (i > atrial.nPages)
        ...
        if ismember( i, atrial.userDefined(1:2) )
            %PRESENTATION WITH OFFSET
            Screen('DrawTexture', windowPtr, Stimuli.tex(atrial.pageNumber(i)), [],  atrial.userDefined(3:6));
        else
