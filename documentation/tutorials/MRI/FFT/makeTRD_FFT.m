function [TRDfileName, Cfg] = makeTRD_FFT(subjectID, session, run, Cfg)
%function TRDfileName = makeTRD_FFT(subject, session, run, Cfg)
%
...
%
%   EXAMPLE CALL
%      TRDfileName = makeTRD_FFT('Demo', 1, 1, Cfg)
if ~isfield(Cfg, 'nRep'), Cfg.nRep = 3; else end;
Cfg.blankPic = 2;
Cfg.Dur.blankTrial = 16;
Cfg.TR=2;
Cfg.expID = 'FFT';
Cfg.factorialStructure = 3;
Cfg.factorNames = {'EFFECTOR'};
Cfg.levelNames = {'FOOT', 'FINGER', 'TONGUE'};
% Make the experimental design
[TrialDefinitions, Cfg] = makeDesign(Cfg);

% Randomization
TrialDefinitions = shuffle(TrialDefinitions);

nTrials = numel(TrialDefinitions);
for iTrial = 1:nTrials
    TrialDefinitions(iTrial).tOnset = (iTrial-1)*16;
end
...
function [TrialDefinitions, Cfg] = makeDesign(Cfg)
...
trialCounter = 0;
condNames = {'FOOT_LEFT', 'FOOT_RIGHT', 'FINGER_LEFT', 'FINGER_RIGHT', 'TONGUE_LEFT', 'TONGUE_RIGHT'};
condCodes = 1:6;
pageVec = [3, 4, 5, 6, 7, 8];
trialCounter = 0;
...
for iFac1 = 1:nFac1
    ...
    for iRep = 1:Cfg.nRep
        ...
        ThisTrial.condName = condNames{iFac1};
        
        % This trial code (ThisTrial.code)
        ThisTrial.code = condCodes(iFac1);
        ...
        ThisTrial.pages = pageVec(iFac1);
        
        % These pages' durations (ThisTrial.durations)
        ThisTrial.durations = 1;
...
for iTrial = 1:nTrials
    idx =  iTrial * 2;
    TrialDefinitionsNew(idx) = TrialDefinitions(iTrial);
    TrialDefinitionsNew(idx).tOnset = idx*16;
    TrialDefinitionsNew(idx+1) = blankTrial;
    TrialDefinitionsNew(idx+1).tOnset = (idx+1)*16;
    
end
idx = numel(TrialDefinitionsNew);
TrialDefinitionsNew(end+1) = blankTrial;
TrialDefinitionsNew(end).pages = 1; %EMPTY PAGE
TrialDefinitionsNew(end).tOnset = idx*16;

for iTrial = 1:numel(TrialDefinitionsNew)
    TrialDefinitionsNew(iTrial).tOnset = (iTrial-1)*16;
end
...
%file unique)
TRDfileName = sprintf('%s-%s-%d-%d.trd', Cfg.expID, subjectID, session, run);
fprintf(1, 'WRITING %s\n', TRDfileName);
