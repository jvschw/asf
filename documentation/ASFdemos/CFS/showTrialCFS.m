function TrialInfo = showTrialCFS(atrial, windowPtr, Stimuli, Cfg)
...
drawWithMask = atrial.userDefined(5:8);
...
for iPage = 1:atrial.startRTonPage-1
    if (iPage > atrial.nPages)
        ...
        if drawWithMask(iPage)
            timing(iPage, 1:6) = drawStimAndMask(windowPtr, Stimuli.tex(atrial.pageNumber(iPage)), destinationRect, atrial.pageDuration(iPage));
        else
            %PUT THE APPROPRIATE TEXTURE ON THE BACK BUFFER
            Screen('DrawTexture', windowPtr, Stimuli.tex(atrial.pageNumber(iPage)), [], destinationRect);
            %drawMask (windowPtr, 50);
...
function [VBLTimestamp, StimulusOnsetTime, FlipTimestamp, Missed, Beampos] = drawStimAndMask (windowPtr, aTexture, destinationRect, nFrames)
nRects = 150;
winrect = [321, 1, 640, 480];
dx = 320;
dy = 0;
w=RectWidth(winrect);
...
for iFrame = 1:nFrames
    colors = ceil(rand(nRects, 3)*255);
    posx = rand(nRects, 1) * w;
    posy = rand(nRects, 1) * h;
    myrect = floor([dx + posx, dy + posy, dx + posx + rand(nRects, 1) * sizeX, dy + posy + rand(nRects, 1) * sizeY]);
    ...
    %OVERWRITE MASK WITH SINGLE RECTANGLE
    %Screen('FillRect', windowPtr, [255, 0, 0]', winrect);
    Screen('FillRect', windowPtr, colors, myrect);
    %if iFrame == 1
        %on the first frame draw stimulus
...
%     else
%         %nondestructive flip
%         [VBLTimestamp, StimulusOnsetTime, FlipTimestamp, Missed, Beampos] = Screen('Flip', windowPtr, [], 2);
%     end
