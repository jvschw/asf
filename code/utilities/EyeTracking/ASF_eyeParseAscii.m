function [tokenResults, trialInfo] = ASF_eyeParseAscii(fname, tokens)
%function [tokenResults, trialInfo] = ASF_eyeParseAscii(fname, tokens)
%%SEGMENTS EYETRACKING (EYELINK) DATA FROM AN ASF EXPERIMENT INTO TRIALS
%%YOU NEED TO CONVERT THE DATA TO ASCII FIRST USING EDF2ASC
%%ALSO CREATES AN OUTPUT FILE THAT CONTAINS ONLY THE SAMPLES OF THE GAZE
%%POSITIONS (.gaz)
...
%%char(tokenResults(1).lines{:})
%%TO SEE ALL FIXATION EVENTS OF THE EXPERIMENT
%char(tokenResults(2).lines{:})
...
for iToken = 1:nTokens
    tokenResults(iToken).token = tokens{iToken};
    tokenResults(iToken).nFound = 0;
    tokenResults(iToken).lines = [];
end
...
%OPEN FILE TO STORE GAZE SAMPLES
[strPath, strName] = fileparts(fname);
fNameSample = [strName, '.gaz'];
...
while 1
    ...
    if ~isempty(str2num( strtok(tline)))
        ...
        for iToken = 1:nTokens
            tmp = strfind(tline, tokenResults(iToken).token);
            if not(isempty(tmp))
                tokenResults(iToken).nFound = tokenResults(iToken).nFound + 1;
                tokenResults(iToken).lines{tokenResults(iToken).nFound} = tline;
                %display(tokenResults(iToken).lines(tokenResults(iToken).nFound))
            end
        end
    end
end
fprintf(1, '\n\n');
%tokenResults.samples = sample;
fclose(fidSample);
fclose(fid);
...
%TRIALEND
%PAGE
msgToken = find(ismember({tokenResults.token}, 'MSG'));
trialNum = 0;
pageCount = 0;
if ~isempty(msgToken)
    for i = 1:tokenResults(msgToken).nFound
        thisLine = tokenResults(msgToken).lines{i};
...
%FIXATIONS
msgToken = find(ismember({tokenResults.token}, 'EFIX'));
if ~isempty(msgToken)
    for i = 1:tokenResults(msgToken).nFound
        thisLine = tokenResults(msgToken).lines{i};
...
%SACCADES
msgToken = find(ismember({tokenResults.token}, 'ESACC'));
if ~isempty(msgToken)
    for i = 1:tokenResults(msgToken).nFound
        thisLine = tokenResults(msgToken).lines{i};
        [dummy, rmdr]  = strtok(thisLine);
...
msgToken = find(ismember({tokenResults.token}, 'EBLINK'));
if ~isempty(msgToken)
    for i = 1:tokenResults(msgToken).nFound
        thisLine = tokenResults(msgToken).lines{i};
